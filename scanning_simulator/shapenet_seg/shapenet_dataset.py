...
import pytorch3d.datasets.shapenet.shapenet_core
import trimesh
import torch

pytorch3d_file = inspect.getfile(pytorch3d.datasets.shapenet.shapenet_core)
SYNSET_DICT_DIR = Path(pytorch3d_file).resolve().parent


class ShapeNetCoreWithSplit(ShapeNetBase):  # pragma: no cover
    """
    ...
    def __init__(
        ...
        Args:
            ...
            split_csv_path: Path to csv containing official ShapeNet splits
        """
        super().__init__()
        ...
        # self.model_dir = "model_watertight.off" if version == 1 else "models/model_normalized.obj"
        ...
        # load info on data train/val/test splits
        if split_csv_path:
            data_split_csv = pandas.read_csv(split_csv_path)
            split_model_ids = set(data_split_csv[data_split_csv['split'] == split]['modelId'].to_numpy().tolist())
        else:
            split_model_ids = None
        ...
        for synset in synset_set:
            ...
            for model in os.listdir(path.join(data_dir, synset)):
                ...
                if split_model_ids is not None:
                    if model not in split_model_ids:
                        continue
    ...
    def load_off_file_by_model_id(self, synset_id, model_id):
        model = {}
    ...
    def load_obj_file_by_model_idx(self, synset_id, model_id):
        model = {}
        model["synset_id"] = synset_id
        model["model_id"] = model_id
        model_path = path.join(
